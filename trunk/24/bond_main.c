...
static int bond_ioctl_break(struct bonding *bond)
{
	...
	while(!skb_queue_empty(&bond->pbuff)) {
		...
		iph = ip_hdr(pb);
		printk("bond_ioctl_break: Releasing packet from %ld\n", (long)ntohl(iph->saddr));
		pb->dev = bond->curr_active_slave->dev;

		printk("bond_ioctl_break: skb count = %d (should be 1)\n", atomic_read(&pb->users));
...
int bond_hoolock_recv(struct sk_buff *skb, struct net_device *dev, struct packet_type* ptype, struct net_device *orig_dev)
{
	...
	__be32 curr_ip;
	read_lock(&bond->lock);
	curr_ip = bond->master_ip;
	read_unlock(&bond->lock);
	/* Don't buffer if the packet is being transmitted
	 * Note: This function gets called even for xmit because we've registered it as ptype_all
	 */
	if(((__be32)ntohl(iph->saddr)) == (__be32)(ntohl(curr_ip))) {
		//return ret;
		goto RETURN_FREE;
	}
...
static void bond_register_hoolock(struct bonding *bond)
{
	...
	printk("Registering HOOLOCK packet type\n");
	/* initialize packet type */
	pk_type->type = htons(ETH_P_ALL);	//XXX - Do we want to buffer _every_ packet received?
	pk_type->dev = bond->dev;
...
static int bond_check_params(struct bond_params *params)
{
	...
	if(bond_mode == BOND_MODE_HOOLOCK) {
		printk(KERN_INFO DRV_NAME
				": Mode set to HOOLOCK\n");
	}
